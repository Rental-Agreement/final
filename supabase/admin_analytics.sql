-- Admin analytics RPC to power dashboard (run in Supabase SQL Editor)
BEGIN;
...
DECLARE
  ...
  v_leases_series JSONB;
  v_disputes_series JSONB;
BEGIN
  ...
  -- Helper series of last 12 months
  WITH months AS (
    ...
    SELECT to_char(m.month, 'YYYY-MM') AS label, COALESCE(COUNT(u.user_id), 0)::int AS value
    FROM months m
